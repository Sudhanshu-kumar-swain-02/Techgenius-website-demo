...
function Counter({ end, suffix = "", duration = 2000 }) {
  ...
  return <span ref={ref} className="hero-number">{val.toLocaleString()}{suffix}</span>;
}
...
const features = [
  { icon: "📋", title: "Drag & Drop Course Builder", desc: "Build rich courses with video, quizzes, PDFs and SCORM in minutes — no code needed.", tag: "Content" },
  { icon: "🎥", title: "Live Virtual Classrooms", desc: "Zoom, Teams and built-in rooms with interactive whiteboards and breakout sessions.", tag: "Collaboration" },
  { icon: "📊", title: "Advanced Analytics", desc: "Real-time dashboards tracking progress, completion rates and learning ROI.", tag: "Insights" },
  { icon: "🎓", title: "Certificates & Credentials", desc: "Branded certificates with QR verification, PDF export and LinkedIn sharing.", tag: "Recognition" },
  { icon: "📱", title: "Mobile Learning App", desc: "Native iOS and Android apps with true offline learning and auto-sync.", tag: "Mobility" },
  { icon: "🌐", title: "30+ Language Support", desc: "Full RTL support and localisation for global learning delivery at scale.", tag: "Global" },
  { icon: "🎨", title: "White-Label Branding", desc: "Custom domain, colours and learner portals that feel like yours — not ours.", tag: "Brand" },
  { icon: "🧠", title: "AI-Powered Assessments", desc: "Auto-grade assignments, generate adaptive quizzes and surface personalized paths.", tag: "Comming Soon" },
  { icon: "🏆", title: "Gamification Engine", desc: "Badges, leaderboards and reward systems that sustainably boost engagement.", tag: "Comming Soon" },
];
...
export default function LMSAbout() {
  ...
  useEffect(() => {
    const t = setInterval(() => setCurrent(p => (p + 1) % screenshots.length), 4000);
    return () => clearInterval(t);
  }, []);
  ...
  return (
    <div className="lms-root bg-white min-h-screen text-gray-900 overflow-x-hidden">
      ...
      {/* ── HERO ── */}
      <section ref={heroRef} className="relative min-h-screen flex flex-col justify-center overflow-hidden " id="lms">
        ...
        <div className="absolute bottom-20 right-1/4 w-80 h-80 bg-purple-300/30 rounded-full blur-3xl"
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] max-w-[85vw] max-h-[85vw] rounded-full border border-blue-500/10 pointer-events-none"
        ...
        <div className="w-full px-5 sm:px-8 lg:px-12 xl:px-16 2xl:px-24 3xl:px-40 py-20 grid lg:grid-cols-2 gap-12 xl:gap-16 items-center relative z-10">
          ...
          <div className={heroIn ? "animate-left" : "opacity-0"}>
            ...
            <h1 className="lms-display text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-700 leading-[1.05] tracking-tight">
              Learning that{" "}
              <span className="shimmer-text">scales</span>
              <br />
              with your{" "}
            ...
            <p className="mt-6 text-gray-500 text-base sm:text-lg leading-relaxed max-w-lg">
              Deliver world-class learning across any device, in any language, at any scale.
              Backed by AI-powered insights and 99.9% uptime SLA.
            </p>
            ...
            <div className="mt-10 flex flex-wrap items-center gap-4 sm:gap-6 text-xs text-gray-500">
              <span className="flex items-center gap-1.5"><span>🔒</span> SOC 2 Type II</span>
              <span className="flex items-center gap-1.5"><span>✅</span> GDPR compliant</span>
              <span className="flex items-center gap-1.5"><span>📱</span> iOS & Android</span>
              <span className="flex items-center gap-1.5"><span>🌐</span> 30+ languages</span>
            </div>
          </div>

          {/* RIGHT screenshot 3D slider */}
          <div className={`screenshot-wrap ${heroIn ? "animate-right" : "opacity-0"}`} style={{ animationDelay: "0.2s" }}>
            <div className="screenshot-inner relative">
              <div className="absolute -inset-4 rounded-3xl blur-2xl opacity-40 transition-all duration-700"
                ...
                <div className="relative h-[320px] sm:h-[380px] lg:h-[420px] overflow-hidden">
                  ...
                  <div className="absolute bottom-4 left-4 right-4 bg-white/85 backdrop-blur-xl border border-white/60 rounded-2xl p-4 shadow-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-500 text-gray-900">{slide.title}</span>
                    </div>
                    <p className="text-xs text-gray-600 mt-1 ml-5">{slide.desc}</p>
                  </div>
                </div>
                ...
                <div className="flex justify-center gap-2 py-3 bg-white">
                  {screenshots.map((_, i) => (
                    <button key={i} onClick={() => setCurrent(i)} className="rounded-full transition-all duration-300"
                      style={{ width: i === current ? "24px" : "6px", height: "6px", background: i === current ? slide.accent : "rgba(0,0,0,0.15)" }} />
                  ))}
                </div>
              </div>
              ...
              <div className="absolute -right-4 sm:-right-8 bottom-1/3 bg-[#0D1627] border border-white/10 rounded-2xl p-3 shadow-xl animate-float" style={{ animationDelay: "1.5s" }}>
                <div className="text-xs text-white/40">Active learners</div>
                <div className="text-lg sm:text-xl lms-display text-blue-400 mt-0.5">10K+</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="border-y border-gray-200 bg-gray-50 py-10">
        <div className="w-full px-5 sm:px-8 lg:px-12 xl:px-16 2xl:px-24 3xl:px-40 text-center">
          <p className="text-xs text-gray-400 uppercase tracking-widest mb-8">Trusted by teams at</p>
          <div className="flex flex-wrap justify-center items-center gap-8 sm:gap-10 opacity-50 grayscale">
            {clients.map((src, i) => (
              <img key={i} src={src} alt="" className="h-5 sm:h-6 object-contain" />
            ))}
          </div>
        </div>
      </div>
      ...
      <section ref={statsRef} className="py-20 sm:py-24 relative">
        <div className="w-full px-5 sm:px-8 lg:px-12 xl:px-16 2xl:px-24 3xl:px-40">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-8">
            {[
              { val: 500, suffix: "K+", label: "Learners worldwide", color: "#3B82F6" },
              { val: 1500, suffix: "+", label: "Institutions onboarded", color: "#8B5CF6" },
              { val: 30, suffix: "+", label: "Languages supported", color: "#F59E0B" },
              { val: 99.9, suffix: "%", label: "Platform uptime SLA", color: "#10B981" },
            ].map((s, i) => (
              <div key={i}
                ...
                <div className="lms-display text-4xl sm:text-5xl font-700" style={{ color: s.color }}>
      ...
      <section ref={featRef} className="py-20 sm:py-24 relative">
        ...
        <div className="w-full px-5 sm:px-8 lg:px-12 xl:px-16 2xl:px-24 3xl:px-40">
          <div className={`text-center mb-16 sm:mb-20 ${featIn ? "animate-up" : "opacity-0"}`}>
            <p className="text-blue-600 text-xs uppercase tracking-widest mb-4">Platform capabilities</p>
            ...
            <p className="text-gray-500 mt-4 max-w-2xl mx-auto text-sm sm:text-base">
              From first course to fifty-thousand learners — all the tools you need without the integrations you dread.
            </p>
          </div>
  ...
  {features.map((f, i) => (
    ...
    >
      ...
      <section ref={testRef} className="py-20 sm:py-24 relative">
        <div className="w-full px-5 sm:px-8 lg:px-12 xl:px-16 2xl:px-24 3xl:px-40">
          <div className={`text-center mb-12 sm:mb-16 ${testIn ? "animate-up" : "opacity-0"}`}>
            <p className="text-blue-600 text-xs uppercase tracking-widest mb-4">Customer stories</p>
            <h2 className="lms-display text-3xl sm:text-4xl font-700">What our clients say</h2>
          </div>
            ...
            {testimonials.map((t, i) => (
              <div key={i}
                ...
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <div className="text-sm text-gray-900 font-500">{t.name}</div>
                    <div className="text-xs text-gray-400">{t.role}</div>
                  </div>
                </div>
                <p className="text-gray-600 text-xs sm:text-sm leading-relaxed">"{t.quote}"</p>
        ...
        <div className="relative">
          ...
          <div className="relative rounded-[40px] border border-gray-200 bg-gray-50 p-10 sm:p-16 text-center overflow-hidden noise-overlay">
            {["-top-2 -left-2", "-top-2 -right-2", "-bottom-2 -left-2", "-bottom-2 -right-2"].map((pos, i) => (
              <div key={i} className={`absolute ${pos} w-4 h-4 rounded-full bg-blue-500 opacity-60 animate-pulse`} />
            ))}
            <p className="text-blue-600 text-xs uppercase tracking-widest mb-4">Get started today</p>
            ...
            <p className="text-gray-500 max-w-xl mx-auto mb-10 text-sm sm:text-base">
              Join 1 500+ institutions already delivering better learning outcomes. Start free, scale when you're ready.
            </p>
            ...
            <p className="text-gray-400 text-xs mt-6">No credit card · 14-day trial · Cancel anytime</p>
          </div>
        </div>
      </section>
